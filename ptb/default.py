...
class RNNForLM(chainer.Chain):
    ...
    def __init__(self, n_vocab, n_units, inner_node, search=True, sharing=False):
        super(RNNForLM, self).__init__()
        with self.init_scope():
            self.embed = L.EmbedID(n_vocab, n_units)
            self.l1 = Cell(n_units, inner_node=inner_node, search=search, sharing=sharing)
    ...
    def __call__(self, x):
        h0 = self.embed(x)
        h1 = self.l1(F.dropout(h0, 0.5))
        y = self.l2(F.dropout(h1, 0.5))
        return y
...
def main(mode, dump_dir):
    ...
    interval = 10 if args.test else 10
    ...
    if mode == 'A':
        options = {'zero_pos':0, 'prior':'uniform'}
    else:
        options = {'zero_pos':None, 'prior':'uniform'}

    npos = sum([i for i in range(1,INNER_NODE+1)])
    normal = util2.Param(mode=mode, shape=(npos,5), max_iter=(args.epoch)*(26568/args.batchsize), options=options, mode2="CAT")
